      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-carefd-slate mt-1">ניהול ספקים מובלטים בתוצאות החיפוש</p>
          </div>
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-2 px-4 py-2 bg-amber-600 text-carefd-navy rounded-lg hover:bg-amber-700 transition"
          >
            <FiStar size={18} />
            הבלט ספק
          </button>
          ...
          <div className="bg-white rounded-xl p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-carefd-navy">{featuredProviders.filter(f => f.is_active).length}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-carefd-slate text-sm">פעילים</p>
                <p className="text-2xl font-bold text-carefd-navy">{featuredProviders.filter(f => f.is_active && getDaysRemaining(f.end_date) > 0).length}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-carefd-slate text-sm">פג תוקף</p>
                <p className="text-2xl font-bold text-carefd-navy">{featuredProviders.filter(f => getDaysRemaining(f.end_date) <= 0).length}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-right py-4 px-6 text-carefd-slate font-medium text-sm">סוג</th>
                  ...
                  return (
                    <tr key={featured.featured_id} className="border-b border-gray-100/50 hover:bg-gray-50/30 transition">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-amber-500 to-orange-600 rounded-full flex items-center justify-center text-carefd-navy font-medium">
                            {featured.provider_name?.[0] || 'P'}
                          </div>
                          <div>
                            <Link 
                              to={`/providers/${featured.provider_id}`}
                              className="text-carefd-navy font-medium hover:text-amber-400"
                            >
                      ...
                      <td className="py-4 px-6">
                        <div className="text-carefd-slate text-sm flex items-center gap-1">
                          <FiCalendar size={14} />
                          {new Date(featured.start_date).toLocaleDateString('he-IL')} - {new Date(featured.end_date).toLocaleDateString('he-IL')}
                        </div>
                        {!isExpired && (
                          <p className={`text-xs mt-1 ${isExpiringSoon ? 'text-amber-400' : 'text-carefd-gray'}`}>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-lg border border-gray-100">
            ...
            <div className="p-6 space-y-4">
                  ...
                  filteredProviders.map((provider) => (
                    ...
                    >
                      ...
                      <div className="text-right flex-1">
                        ...
                        <p className="text-carefd-slate text-sm">{provider.provider_type}</p>
                      </div>
                      {selectedProvider?.provider_id === provider.provider_id && (
              ...
              {selectedProvider && (
                <div>
                  <label className="block text-sm text-carefd-slate mb-2">תקופת הבלטה</label>
                  <select
            ...
            <div className="flex items-center justify-end gap-3 p-4 border-t border-gray-100">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-amber-600 text-carefd-navy rounded-lg hover:bg-amber-700 transition disabled:opacity-50"
              >
                <FiStar size={18} />
                הבלט
              </button>
