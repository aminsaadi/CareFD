...
const AdminFeatured = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [featuredRes, providersRes] = await Promise.all([
        api.get('/admin/featured'),
        api.get('/providers?limit=200')
      ]);
      setFeaturedProviders(featuredRes.data.providers || []);
  ...
  const addFeatured = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'שגיאה בהבלטת ספק');
    } finally {
  ...
  const removeFeatured = async (provider) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'שגיאה בהסרת הבלטה');
    }
  };
