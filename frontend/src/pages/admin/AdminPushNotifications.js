...
const AdminPushNotifications = () => {
  ...
  const getTargetIcon = (target) => {
    switch (target) {
      case 'all': return <FiUsers className="text-carelink-teal" />;
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-carelink-navy">התראות Push</h1>
            <p className="text-carelink-slate mt-1">שליחת התראות למשתמשים</p>
          </div>
          <button
            onClick={() => setShowSendModal(true)}
            className="flex items-center gap-2 px-4 py-2 bg-carelink-teal text-white rounded-lg hover:bg-carelink-teal/90 transition shadow-md"
          >
            <FiSend size={18} />
            שלח התראה חדשה
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-carelink-teal to-carelink-navy text-white rounded-xl p-5">
          ...
          <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="font-medium text-carelink-navy">לספקים</span>
            </div>
            <p className="text-sm text-carelink-slate">
          ...
          <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="font-medium text-carelink-navy">למשתמשים</span>
            </div>
            <p className="text-sm text-carelink-slate">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50/50">
            <h2 className="font-semibold text-carelink-navy">היסטוריית התראות</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/30">
                  <th className="text-right py-4 px-4 text-carelink-slate font-medium text-sm">כותרת</th>
                  <th className="text-right py-4 px-4 text-carelink-slate font-medium text-sm">תוכן</th>
                  <th className="text-right py-4 px-4 text-carelink-slate font-medium text-sm">יעד</th>
                  <th className="text-right py-4 px-4 text-carelink-slate font-medium text-sm">נמענים</th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="py-12 text-center">
                      ...
                      <p className="text-carelink-gray">עדיין לא נשלחו התראות</p>
                      <button
                        onClick={() => setShowSendModal(true)}
                        className="mt-4 text-carelink-teal hover:underline"
                      >
                        שלח התראה ראשונה
                      </button>
                    </td>
                  ...
                  history.map((notification) => (
                    <tr key={notification.notification_id} className="border-b border-gray-50 hover:bg-carelink-teal-pale/10 transition">
                      <td className="py-4 px-4">
                        <span className="font-medium text-carelink-navy">{notification.title}</span>
                      </td>
                      <td className="py-4 px-4 text-carelink-slate text-sm max-w-xs truncate">
                        {notification.body}
                      </td>
                      <td className="py-4 px-4">
                      ...
                      <td className="py-4 px-4">
                        <span className="inline-flex items-center gap-1 text-carelink-teal font-medium">
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl w-full max-w-lg p-6 border border-gray-200 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-carelink-navy mb-1">תוכן *</label>
            <textarea
              value={formData.body}
              onChange={(e) => setFormData({ ...formData, body: e.target.value })}
              placeholder="תוכן ההתראה..."
              rows={4}
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:border-carelink-teal outline-none resize-none"
              required
            />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-carelink-navy mb-2">שלח אל</label>
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex items-center gap-2 p-3 rounded-lg border transition ${
                  formData.target === 'all' 
                    ? 'border-carelink-teal bg-carelink-teal-pale text-carelink-teal' 
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={sending}
              className="flex-1 px-4 py-2.5 bg-carelink-teal text-white rounded-lg hover:bg-carelink-teal/90 transition flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {sending ? (
