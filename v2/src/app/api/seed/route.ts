...
export const POST = withErrorHandler(async (req: NextRequest) => {
    ...
    if (!existingProvider) {
      const provider = await prisma.provider.create({
        data: {
          ...
          about: "בוגרת הפקולטה לרפואה בטכניון, התמחות ברפואת משפחה במרכז הרפואי רמב\"ם. מאמינה בגישה הוליסטית לטיפול רפואי.",
          professionName: "רפואת משפחה",
          professionTitle: "רופאת משפחה",
          specializations: ["גריאטריה", "רפואה מונעת", "מחלות כרוניות"],
          expertise: ["טיפול בקשישים", "סוכרת", "יתר לחץ דם", "רפואה מונעת"],
          city: "תל אביב-יפו",
          ...
          gender: "female",
          targetAudience: ["קשישים", "מבוגרים", "משפחות"],
        },
      });
      results["provider_profile"] = { status: "created", provider_id: provider.id };

      // Create demo services for this provider
      for (const s of DEMO_SERVICES) {
        await prisma.service.create({
          data: {
            ...
            price: s.price,
            serviceCategory: s.category as any,
            pricingType: s.pricingType as any,
            durationMinutes: s.duration,
            deliveryTypes: ["home_visit", "clinic_visit"],
            currency: "ILS",
            isActive: true,
          },
        });
      ...
      results["provider_profile"] = { status: "exists", provider_id: existingProvider.id };
    }
  }

  // Create some professions if none exist
  ...
  if (profCount === 0) {
    ...
    for (const p of profs) {
      await prisma.profession.create({ data: p });
    }
