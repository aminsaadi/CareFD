    ...
    <div className="min-h-screen bg-white" dir="rtl">
      ...
      <section className="bg-gradient-to-br from-carefd-navy to-carefd-teal py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-xl text-carefd-teal-pale">נשמח לשמוע מכם! הצוות שלנו כאן לכל שאלה</p>
        </div>
      </section>
            ...
            <div>
              ...
              {isSubmitted ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-8 text-center">
                  ...
                  <button onClick={() => setIsSubmitted(false)} className="mt-4 text-carefd-teal font-semibold hover:underline">
                    שלח הודעה נוספת
                  </button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg">
                  <div className="grid md:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-carefd-teal focus:outline-none" placeholder="השם שלך" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-carefd-navy mb-2">אימייל *</label>
                      <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-carefd-teal focus:outline-none" placeholder="your@email.com" />
                    </div>
                  </div>
                  <div className="grid md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <label className="block text-sm font-medium text-carefd-navy mb-2">טלפון</label>
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-carefd-teal focus:outline-none" placeholder="050-0000000" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-carefd-navy mb-2">נושא *</label>
                      <select name="subject" value={formData.subject} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-carefd-teal focus:outline-none">
                  ...
                  <div className="mb-6">
                    ...
                    <textarea name="message" value={formData.message} onChange={handleChange} required rows={5} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-carefd-teal focus:outline-none resize-none" placeholder="כתבו את ההודעה שלכם..." />
                  </div>
                  <button type="submit" disabled={isSubmitting} className="w-full bg-carefd-teal text-white py-3 px-6 rounded-xl font-semibold hover:bg-carefd-teal-medium transition flex items-center justify-center gap-2 disabled:opacity-50">
                    {isSubmitting ? (
                      <>
