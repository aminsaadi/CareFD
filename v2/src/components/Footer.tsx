...
export default function Footer() {
  ...
  useEffect(() => {
    api.get<any>("/settings/public").then((d) => setSettings((prev: any) => ({ ...prev, ...d }))).catch(() => {});
    api.get<{ regions: any[] }>("/regions").then((d) => setRegions(d.regions || [])).catch(() => {});
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid md:grid-cols-5 gap-8">
          ...
          <div className="md:col-span-1">
            ...
            <p className="text-carefd-teal-light mb-4 text-sm">Connecting Care Providers</p>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {regions.slice(0, 6).map((r: any, i: number) => (
                <li key={i}>
                  <Link href={`/providers?city=${encodeURIComponent(r.name || r)}`} className="text-carefd-light-gray hover:text-carefd-teal transition">
                    {r.name || r}
                  </Link>
                </li>
              ))}
              <li><Link href="/providers" className="text-carefd-teal hover:text-carefd-teal-light transition text-sm">כל האזורים</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-2 text-carefd-light-gray">
                <Phone className="w-4 h-4 text-carefd-teal" />
                <span dir="ltr">{settings.contact_phone}</span>
              </li>
              {settings.contact_email && (
                <li className="flex items-center gap-2 text-carefd-light-gray">
                  <Mail className="w-4 h-4 text-carefd-teal" />
