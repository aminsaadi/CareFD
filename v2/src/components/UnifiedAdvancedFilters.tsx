  ...
  useEffect(() => {
    api.get<{ professions: { profession_id: string; name: string }[] }>("/professions")
      .then((data) => {
        ...
        if (profs.length > 0) {
          setServiceCategories(profs.map((p) => ({ id: p.profession_id, name: p.name })));
        }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-lg overflow-hidden ${showMobile ? "fixed inset-0 z-50 overflow-y-auto" : ""}`}>
      ...
      <div className="bg-carefd-navy text-white p-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onReset} className="text-sm text-carefd-teal-pale hover:text-white transition">
            נקה הכל
          </button>
          {showMobile && onClose && (
                    ...
                    <div className="p-1">
                      {filteredCities.map((city, index) => (
                        <button
                          ...
                          className={`w-full text-right px-3 py-2 hover:bg-carefd-teal-pale/30 rounded-lg transition-colors flex items-center gap-2 text-sm ${
                            filters.city === city.name ? "bg-carefd-teal text-white" : "text-carefd-navy"
                          }`}
                        >
                          <MapPin className="w-3 h-3" />
                          <span>{city.name}</span>
          ...
          {(filters.city || filters.region) && (
            <div className="flex items-center gap-2 bg-carefd-teal-pale/30 px-3 py-2 rounded-lg">
              ...
              <button
                onClick={() => {
                  setLocationSearch("");
                  onFilterChange({ ...filters, city: "", region: "", latitude: null, longitude: null, radius: null, useMyLocation: false });
                }}
          ...
          {languageOptions.map((lang) => {
            ...
            return (
              <button
                ...
                className={`px-2 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  isSelected ? "bg-carefd-teal text-white" : "bg-carefd-teal-pale/30 text-carefd-navy hover:bg-carefd-teal-pale"
                }`}
              >
                {lang.name}
              </button>
      ...
      {/* Price Filter */}
      <FilterSection title="טווח מחירים" icon={Clock} sectionKey="price">
        <div className="space-y-3">
          <div className="flex gap-2">
